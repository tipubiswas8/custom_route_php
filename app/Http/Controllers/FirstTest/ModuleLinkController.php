...
class ModuleLinkController extends Controller
{
    ...
    {
        $moduleLink = ModuleLinkModel::all();
        return view("first-test.module_link", ['moduleLink' => $moduleLink]);
    }
    ...
    public function linkCreate(Request $request)
    {
        if ($_GET)
            if ($request->link_type == 1) {
                ...
                $step = 2;
                return view("first-test.module_link_create", [
                    'step' => $step,
                    'modules' => $modules,
                ]);
            }
        ...
        if ($request->step == 3) {
            ...
            $step = 3;
            $folder = null;
            $existingController = ModuleLinkModel::select('controller')->where('module_id', $module_id)->first();
            if ($existingController != null) {
                $exController = ModuleLinkModel::select('controller')->where('module_id', $module_id)->first()->controller;
        ...
        if ($request->step == 5) {
            $module_id = $request->module_id;
            $mainMenu = ModuleModel::where(['active_status' => '1', 'type' => '2', 'parent_module_id' => $module_id])->get();
            $step = 5;
            $folder = null;
            $existingController = ModuleLinkModel::select('controller')->where('module_id', $module_id)->first();
            if ($existingController != null) {
        ...
        if ($request->step == 8) {
            ...
            $step = 8;
            $folder = null;
            $subMenu = ModuleModel::where(['active_status' => '1', 'type' => '3', 'parent_module_id' => $module_id, 'parent_menu_id' => $main_menu])->get();
            $existingController = ModuleLinkModel::select('controller')->where('module_id', $module_id)->first();
            if ($existingController != null) {
                $exController = ModuleLinkModel::select('controller')->where('module_id', $module_id)->first()->controller;
                $position = strpos($exController, '\\');
    ...
    public function linkStore(Request $request)
    {
        ...
        if ($module_id_for_other) {
            if ($module_id) {
                ...
                if ($name_for_all) {
                    ...
                    $name_of_module = ModuleModel::select('name')->where('id', $module_id)->first()->name;
            ...
            $existing = ModuleLinkModel::where(['link_type' => '3', 'sub_menu_id' => $sub_menu_id])->first();
            if ($existing) {
            ...
            $sub_menu = ModuleModel::select('name')->where('id', $sub_menu_id)->first()->name;
            ...
            $existing = ModuleLinkModel::where(['link_type' => '2', 'main_menu_id' => $main_menu_id])->first();
            if ($existing) {
            ...
            $main_menu = ModuleModel::select('name')->where('id', $main_menu_id)->first()->name;
            ...
            $existing = ModuleLinkModel::where(['link_type' => '1', 'module_id' => $module_id])->first();
            if ($existing) {
        ...
        $kebab = Str::kebab($folder);
        $folderAndView =  $kebab ? $kebab . '\\' . $view : $view;

        try {
            $insert = ModuleLinkModel::create([
