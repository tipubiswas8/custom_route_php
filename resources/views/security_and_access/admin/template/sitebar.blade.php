...
<div class="col-md-2" style="height: 800px; background-color: #dde39f">
    <div class="text-white" style="height: 100%">
        ...
        <form action="{{ url($moduleRoute->url ?? '#') }}" method="{{ $request_type }}"> 
            ...
            @csrf
           <?= $module->icon ?? '<i class="fa-solid fa-image"></i>' ?><input style="margin-left: -10px" class="mb-0 pb-0 mt-0 pt-0 btn btn-link" type="submit" value="{{ $module->name ?? '' }}">
        </form>
            ...
            {{-- Main menu name and route --}}
                ...
                @endif
                    ...
                    @endif
                @endforeach
             <i style="margin-left: 25px"><?= $mainMenu->icon ?? '<i class="fa-solid fa-image"></i>' ?><input style="margin-left: -10px" class="mb-0 pb-0 mt-0 pt-0 btn btn-link" type="submit" value="{{ $mainMenu->name }}"></i>
                    </form>
                @foreach ($sMenus as $sm)
                    ...
                    {{-- Sub menu name and route --}}
                        ...
                        @endif  
                            ...
                            @endif
                        @endforeach
                        <i style="margin-left: 50px"><?= $sm->icon ?? '<i class="fa-solid fa-image"></i>' ?><input style="margin-left: -10px" class="mb-0 pb-0 mt-0 pt-0 btn btn-link" type="submit" value="{{ $sm->name }}"></i>
                                </form>
                    @endif
                @endforeach
            @endforeach
